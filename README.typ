...
#align(center)[
  #text(size: 24pt, weight: "bold")[Sanskar's NixOS Profile]
  #text(size: 14pt, style: "italic")[Stable • Fast Boot • NVIDIA Gaming • X11 Plasma]
]

#v(0.6cm)

= Overview
This repository is a stability-first NixOS configuration for a hybrid AMD + NVIDIA laptop. The focus is fast boot, predictable behavior, and reliable NVIDIA gaming. It uses *Nix Flakes* and *Home Manager* so system and user settings rebuild cleanly from source.
...
- *auto-cpufreq* disabled for stability.

== Storage
- ext4 `noatime` preserved.
- SSD trim enabled via `fstrim`.
- zram swap enabled.
...
- Blueman GUI (Bluetooth core remains enabled)
